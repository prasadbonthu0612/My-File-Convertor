...
def make_branding_overlay(job_id, logo_path, share_icon_path=None, follow_icon_path=None):
    """Create one small transparent branding layer for the FFmpeg job.

    V10 fed the logo, share icon, and follow icon to FFmpeg as three separate
    looping video inputs. That creates multiple framesync/overlay paths and
    several decoded image streams. V11 pre-composes those static graphics into
    one transparent PNG. The actual video, audio, title drawtext, encoding,
    queue worker, and live progress architecture remain unchanged.

    The overlay is only as tall as the branding header, not 720x1280, so the
    image decoder never has to carry a full-frame transparent canvas.
    """
    ...
    overlay_height = (share_y + icon_size + 4) if SHOW_SHARE_FOLLOW else (CHANNEL_LOGO_TOP + logo_max_height)
    ...
    if SHOW_SHARE_FOLLOW:
        ...
        # Match V10's FFmpeg overlay coordinates exactly. FFmpeg's
        # force_original_aspect_ratio=decrease changes the icon frame itself
        # (for example 260x120 -> 42x19), then overlays that frame at y=share_y;
        # it does NOT vertically center the reduced frame in a 42x42 box.
        share_x = share_icon_x
        follow_x = follow_icon_x

        canvas.alpha_composite(share_icon, (share_x, share_y))
        canvas.alpha_composite(follow_icon, (follow_x, share_y))
...
async def convert_mkv_to_mp4(
    ...
    branding_overlay_path = make_branding_overlay(
        job_id,
        logo_path,
        share_icon_path,
        follow_icon_path,
    ...
    # branding layer. The title remains drawtext so Telugu/special characters
    # keep the same behavior as V10.
    video_filter = (
        ...
        "[base][brand]overlay="
        "x=0:y=0:eof_action=repeat:format=auto[branded];"
        "[branded]drawtext="
        ...
        "enable='between(t,0,999999)'[vout]"
    )
