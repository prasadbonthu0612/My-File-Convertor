...
def _title_layout(title):
    """Render the title at the fixed code-defined size and wrap it if needed."""
    max_width = max(200, min(680, TITLE_TEXT_MAX_WIDTH))
    # IMPORTANT: Never reduce the title font size based on title length or
    # Render environment variables. Keep the configured 52px size and wrap
    # onto additional lines when necessary.
    return TITLE_FONT_SIZE, _wrap_title_lines(title, TITLE_FONT_SIZE, max_width)
