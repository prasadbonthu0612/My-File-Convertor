print("🔥 MKV CONVERTER V9 - MEMORY-SAFE RENDER + LIVE WORKFLOW REPORT + BRANDING")
...
# Render Free has 512 MB RAM and a very small CPU allocation. Keep FFmpeg's
# thread pools bounded so one conversion does not create avoidable memory
# pressure. The architecture remains one queue worker + one FFmpeg process.
FFMPEG_THREADS = max(1, min(2, int(os.environ.get("FFMPEG_THREADS", "1"))))
FFMPEG_FILTER_THREADS = max(
    1,
    min(2, int(os.environ.get("FFMPEG_FILTER_THREADS", "1"))),
)
        ...
        while True:
            ...
            if now - last_report >= 4:
                ...
                text = (
                    ...
                    f"(filters: {FFMPEG_FILTER_THREADS})\n\n"
