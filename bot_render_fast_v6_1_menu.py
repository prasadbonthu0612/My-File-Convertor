...
async def help_handler(event):
    await event.respond(
        "🤖 MKV Converter Bot\n\n"
        "Send one or more .mkv files. They are placed into a FIFO queue "
        ...
        "/cancel ID — cancel a waiting job or current conversion\n"
        "/retry ID — retry a failed job\n"
        "/delete ID — delete one completed output message\n"
...
async def cancel_handler(event):
    ...
    if not raw_id:
        await event.respond("Usage: /cancel ID\nExample: /cancel 3")
        return
    ...
    if job["state"] == "queued":
        job["state"] = "cancelled"
        await event.respond(f"🛑 Job #{job_id} cancelled before processing.")
        return

    if job["state"] == "processing":
        state = job.get("conversion_state")
        ...
        job["state"] = "cancelled"
        await event.respond(f"🛑 Job #{job_id} cancellation requested.")
        return

    await event.respond(
        f"ℹ️ Job #{job_id} is already {job['state']} and cannot be cancelled."
    )
...
async def process_job(job_id):
    ...
    try:
        ...
        if not message or not message.file:
            raise RuntimeError("Original Telegram MKV message is no longer available.")
        ...
        await update_progress(
            ...
            f"🔍 Job #{job_id}\n\n"
            "MKV downloaded.\n"
            "Preparing 720×1280 Telugu conversion..."
        )
