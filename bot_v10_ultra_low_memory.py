print("🔥 MKV CONVERTER V10 - ULTRA LOW-MEMORY RENDER + LIVE WORKFLOW REPORT + BRANDING")
...
async def convert_mkv_to_mp4(
    ...
    # LOW-MEMORY FILTERGRAPH
    #
    # Seek to the requested beginning first and stop the output after exactly
    # the requested kept duration. The old graph decoded the complete source
    # up to end_cut and then discarded the intro; this graph avoids decoding
    # media that can never appear in the output.
    #
    # The video still goes through the same single FFmpeg process and the same
    # 720x1280 canvas/branding pipeline. Only the FFmpeg graph is simplified.
    video_filter = (
    ...
    # One source input + the existing static branding inputs. Each demuxer gets
    # a tiny packet queue so a slow filter/encoder cannot accumulate packets in
    # RAM. Input seek/timing is used instead of trim/atrim filters.
    command = [
        "ffmpeg",
        "-y",
        "-hide_banner",
